...
getPriceIndex also returns coal_price (动力煤) and mjc (水泥煤价差), but
those are not stored here — only cement is wired up.
...
def fetch_series() -> tuple:
    ...
    Returns
    -------
    (cempi_points, po425_points) where each is a list of (date_str, value)
    ...
    cement = j["Data"]["cement"]
    cempi = list(zip(cement["dynamicIndexDate"], cement["dynamicIndexAll"]))
...
def main() -> int:
    ...
    try:
        with conn.cursor() as cur:
            ...
            # ── 3. fetch_log（商品侧惯例：commodities 路线才记） ─────────────
            change = None
            if len(po425) >= 2:
